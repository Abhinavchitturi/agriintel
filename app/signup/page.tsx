...
function SignUpContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    setError("")
    const result = await dbSignUp(email.trim(), password, name.trim())
    setIsLoading(false)
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center px-4 relative overflow-hidden">
      ...
      >
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            {wantsPro ? "Start your Pro plan" : "Create your account"}
          </h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">
            {wantsPro
              ? "Sign up and add payment to unlock Pro features"
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-8 shadow-xl">
          ...
          <div className="mt-6 text-center text-sm text-gray-500 dark:text-gray-400">
            ...
            <Link href="/login" className="text-agri-600 dark:text-agri-400 hover:underline font-medium">
              Sign in
            </Link>
          </div>
        </div>
      ...
      <PaymentModal
        ...
        onClose={() => { setShowPayment(false); router.replace("/chat") }}
